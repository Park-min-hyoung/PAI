...
스택을 이용한 문자 제거(v)
1. Counter 클래스를 사용해 현재 문자를 기준으로 뒤쪽에 스택에 최근에 입력된 문자가 있는지 check 할 수 있으며, set을 통해
현재 문자가 set 있으면 다음 반복문으로 갈 수 있게끔 해 중복을 피할 수 있다.
3. 현재 문자가 최근 스택에 쌓인 문자보다 사전순으로 빠르고 스택에 쌓인 문자가 아직 반복되지 않은 문자열 원소 중에(현재 문자 뒤쪽)
있다면 스택에서 그 문자를 빼낸다.'''