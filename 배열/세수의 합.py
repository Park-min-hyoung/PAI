...
배열에 추가해 준다
4. 배열에 추가한 후 중복 되는 배열 원소는 더해주지 않기 위해 left와 right를 수정해준다(만약 0 -1 -1 -1 1 라는 리스트가 있다면
...
5. start에 해당하는 반복문을 돌때 nums의 현재 인덱스에 해당하는 값이 그전의 인덱스에 해당하는 값과 같다면 다음 index로 넘어간다
(어차피 값이 같으면 목표값을 찾더라도 결과 배열에 추가되는 원소는 같기 때문에 중복된다)
'''